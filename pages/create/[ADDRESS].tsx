...
export default function Create(){
  ...
  const handleNext = () => {
    ...
    if(wallet){
      ...
      if(step === 2){
        // We should create the deal there
        router.push('/');
      }
    }
  ...
  return (
    <Container maxWidth="lg">
        ...
        {step < 2 ? (
            <>
              <h1 className="mb-0">{step === 0 ? 'Select their items' : 'Select your items'}</h1>
              ...
              <div className={styles.dealTopContainer}>
                <h1>Deal to {tradeUser.name}#{tradeUser.address.substr(2, 4)}</h1>
              </div>
              <div className={styles.dealBottomContainer}>
                <div className={styles.dealBottomLeftContainer}>
                  <div>
                    <h2 className="mb-0">{user.name}#{user.address.substr(2, 4)}</h2>
                    <span className="mt-0">{deal[1].assets.length} item{deal[1].assets.length > 1 ? 's' : ''}</span>
                  </div>
                ...
                <div className={styles.dealBottomRightContainer}>
                  <div>
                    <h2 className="mb-0">{tradeUser.name}#{tradeUser.address.substr(2, 4)}</h2>
                    <span className="mt-0">{deal[0].assets.length} item{deal[1].assets.length > 1 ? 's' : ''}</span>
                  </div>
                  <div className={styles.dealNftContainer}>
                    <NFTGrid
                      data={deal[0].assets}
